...
class Flat(Figure):
    ...
    def plot(sef, coords=coords):
        ...
        ax.set_aspect("equal")
        ax.plot(
            [coord[0] for coord in coords], [coord[1] for coord in coords], color="teal"
        )
        buf = io.BytesIO()
...
class Rectangle(Flat):
    ...
    def get_square(self):
        self.output[("Площадь", "square")] = (
            self.input[("Сторона А", "a_side")] * self.input[("Сторона Б", "b_side")]
        )
...
class Trapezoid(Flat):
    ...
    def __init__(self, a=5, b=4, c=3, h=3):
        super().__init__()
        self.input = {
            ("Нижнее основание", "low_base"): a,
            ("Боковая сторона", "side"): b,
            ("Верхнее основание", "high_base"): c,
            ("Высота", "hight"): h,
        }
        leg = np.sqrt(b**2 - h**2)
    ...
    def get_square(self):
        base1, base2, h = (
            self.input[("Нижнее основание", "low_base")],
            self.input[("Верхнее основание", "high_base")],
            self.input[("Высота", "hight")],
        )
...
class Rhomb(Flat):
    ...
    def __init__(self, d1=5, d2=3):
        super().__init__()
        self.input = {
            ("Большая диагональ", "bg_diagonal"): max(d1, d2),
            ("Малая диагональ", "sm_diagonal"): min(d1, d2),
        }
    ...
    def get_square(self):
        a, b, c = (
            self.output[("Сторона", "side")],
            self.input[("Большая диагональ", "bg_diagonal")],
            self.input[("Малая диагональ", "sm_diagonal")],
        )
...
class Circle(Flat):
    ...
    def get_perimeter(self):
        self.output[("Периметр", "perimeter")] = (
            2 * self.input[("Радиус", "radius")] * np.pi
        )

    def get_square(self):
        self.output[("Площадь", "square")] = (
            np.pi * self.input[("Радиус", "radius")] ** 2
        )
...
class Parallelepiped(Volumetric):
    ...
    def get_square(self):
        base_squares = (
            self.input[("Ребро А", "a_side")] * self.input[("Ребро Б", "b_side")]
        ) * 2
        side_squares = (
            self.input[("Ребро А", "a_side")] * self.input[("Ребро В", "c_side")]
        ) * 4
        self.output[("Площадь", "square")] = base_squares + side_squares

    def get_volume(self):
        self.output[("Объём", "volume")] = (
            self.input[("Ребро А", "a_side")]
            * self.input[("Ребро Б", "b_side")]
            * self.input[("Ребро В", "c_side")]
        )
...
class Pyramid(Volumetric):
    ...
    def __init__(self, c=4, l=5, e=6):
        super().__init__()
        self.input = {
            ("Число сторон основания", "base_count"): c,
            ("Длина стороны основания", "base_length"): l,
            ("Ребро", "edge"): e,
        }
        ...
        try:
            ...
            self.output = {
                (
                    "Ошибка",
                    "error",
                ): "Невозможно построить пирамиду с данным набором параметров. Пожалуста, попробуте ещё раз."
            }
    ...
    def get_square(self):
        count, length = (
            self.input[("Число сторон основания", "base_count")],
            self.input[("Длина стороны основания", "base_length")],
        )
    ...
    def get_coords(self):
        ...
        for vertex in range(count):
            coords += [
                (np.cos(alpha) * radius, np.sin(alpha) * radius, 0),
                (0, 0, hight),
                (np.cos(alpha) * radius, np.sin(alpha) * radius, 0),
            ]
            alpha += 2 * np.pi / count
...
class Cone(Volumetric):
    ...
    def get_perimeter(self):
        self.output[("Периметр основания", "base_perimeter")] = self.base.output[
            ("Периметр", "perimeter")
        ]

    def get_square(self):
        self.output[("Площадь основания", "base_square")] = self.base.output[
            ("Площадь", "square")
        ]
    ...
    def get_coords(self):
        ...
        for dot in net:
            cone += [
                (rd, rd, h),
                (dot, (np.sqrt(rd**2 - (dot - rd) ** 2) + rd), 0),
                (rd, rd, h),
                (dot, (-np.sqrt(rd**2 - (dot - rd) ** 2) + rd), 0),
                (rd, rd, h),
            ]
...
class Sphere(Volumetric):
    ...
    def get_perimeter(self):
        self.output[("Периметр сечения", "sec_perimeter")] = self.sec.output[
            ("Периметр", "perimeter")
        ]

    def get_square(self):
        self.output[("Площадь основания", "base_square")] = self.sec.output[
            ("Площадь", "square")
        ]
